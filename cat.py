...
def calculate_daily_sales_and_turnover_single_category(csv_file, final_date, days, category):
    ...
    while current_date <= end_date_obj:
        ...
        category_sales = daily_df[daily_df['product_group_name'].isin(category_products)]['sales_1_days'].sum()

        # Calculate the turnover for the category by multiplying sales with price_per_sale
        category_turnover = (daily_df[daily_df['product_group_name'].isin(category_products)]
                             ['sales_1_days'] * daily_df['price_per_sale']).sum()

        # Find the highest selling product for the category on the current day
        highest_product_df = daily_df[daily_df['product_group_name'].isin(category_products)]
...
category = "Clothing"  # The category to analyze
